...
parameters {
    ...
    matrix[8,2]   theta_mu;                 // Population-level effects (8 params x 2 sessions)
    matrix[8,NJ]  theta_c_pr;              // Standardized subject-level effects (common)
    matrix[8,NJ]  theta_d_pr;              // Standardized subject-level effects (divergent)

    // Parameter variances
    matrix<lower=0>[8,2] sigma;             // Subject-level standard deviations
...
transformed parameters {
    ...
    matrix[NJ,8] theta_c = transpose(diag_pre_multiply(sigma[,1], theta_c_pr));
    matrix[NJ,8] theta_d = transpose(diag_pre_multiply(sigma[,2], theta_d_pr));
...
model {
    ...
    for (n in 1:N) {
        ...
        mu[n] = beta * (Q[J[n],K[n],M[n],2] - Q[J[n],K[n],M[n],1]) + tau + delta_c * C[n];
