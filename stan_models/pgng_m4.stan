// M4: Controllability modulation + lapse rate (M2 + M3)
// Full model: reward sensitivity, go bias, learning rate, controllability delta, lapse rate
...
data {
    ...
    array[N] int<lower=0, upper=1>  C;      // Controllability
...
transformed parameters {
    ...
    vector[NJ]  a1;
    vector[NJ]  a2;
    vector[NJ]  d1;                         // Controllability modulation (reward)
    vector[NJ]  d2;                         // Controllability modulation (punishment)
    vector[NJ]  c1;                         // Lapse rate

    {
    matrix[NJ,9] theta = transpose(diag_pre_multiply(sigma, theta_pr));
    ...
    d1 = (theta_mu[7] + theta[,7]) * 5;
    d2 = (theta_mu[8] + theta[,8]) * 5;
    c1 = Phi_approx(-2.0 + theta_mu[9] + 0.5 * theta[,9]);
    }
...
model {
    ...
    for (n in 1:N) {
        ...
        mu[n] = (0.5 * xi) + (1 - xi) * inv_logit(
            beta * (Q[J[n],K[n],2] - Q[J[n],K[n],1]) + tau + delta_c * C[n]
        );
...
generated quantities {
    ...
    real  d1_mu = theta_mu[7] * 5;
    real  d2_mu = theta_mu[8] * 5;
    real  c1_mu = Phi_approx(-2.0 + theta_mu[9]);
